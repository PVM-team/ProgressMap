ProgressApp.service('ActionMapUpdaterService', function (AssignmentLatestAttemptersService, AssignmentCirclesService, MapScaleService, MoveStudentCircleService, StudentIconService) {
    ...
    var assignments; // muotoa {'id', 'number', latestAttempters: {'id', 'location', 'reserved', 'leaving', 'dummy'}, doers: {'id', ...}}
    ...
    */
    ...
    function placeStudentsOnMapWhichAreNotThereYetButNowShouldBe() {
        ...
        for (var i = 0; i < students.length; i++) {
            ...
            if (lastDoneAssignment) {
                ...
                var originalAssignment = assignments[student.originalAssignment.number - 1];
    ...
    function movingStudents() {
        ...
        for (var i = 0; i < students.length; i++) {
            ...
            if (lastDoneAssignment) {
                var destinationAssignment = assignments[lastDoneAssignment.number - 1];
                var originalAssignment = assignments[student.originalAssignment.number - 1]; // undefined if not shown anywhere in map
    ...
    */
    ...
    function getStudentCircle(student, assignment) {
        var location = AssignmentLatestAttemptersService.getLocationOfStudent(student, assignment);
